...
int												CWFileHandler::initStream(std::string _streamFolder, unsigned int _bytePerFile)
{
	WIN32_FIND_DATA								ffd;
	...
	while (FindNextFile(hFind, &ffd) != 0)
	{
		if (!(ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
		{
			...
			if (!strncmp(ffd.cFileName, ".WINDOWS_", 9))
				match_files[ffd.cFileName] = filesize.QuadPart;
		}
	}
	for (it = match_files.begin(); it != match_files.end(); ++it)
		this->stream.push_back(new CWFile(this->streamFolder + (*it).first, (*it).second));
	FindClose(hFind);
	return (n_files);
}

bool					CWFileHandler::writeFromFile(IFile *current, std::string &raw)
{
	std::time_t			t;
	std::stringstream	tm;
	std::string			tmsp;
	...
	tmsp = tm.str();
	if (!current) {
		current = new CWFile(this->streamFolder + std::string(".WINDOWS_") + tmsp, 0);
...
bool		CWFileHandler::insertDataToStream(std::string raw)
{
	IFile	*file;
...
std::string							CWFileHandler::extractDataFromStream(int nbFile)
{
...
void								CWFileHandler::removeLocalData(int nbFile)
{
	std::list<IFile *>::iterator	file;
	int								i = 0;
...
void								CWFileHandler::closeStream(void)
{
	std::list<IFile *>::iterator	file;
...
void								CWFileHandler::showStream(void)
{
	std::list<IFile *>::iterator	file;
